...
void hardwareD3Para(_In_ RT_ADAPTER* adapter);
void exitOOB(_In_ RT_ADAPTER* adapter);
void powerDownPLL(_In_ RT_ADAPTER* adapter);
void configPhyHardware(_In_ RT_ADAPTER* adapter);
NTSTATUS setMulticastMode(_In_ RT_ADAPTER* adapter, BOOLEAN active);
...
void RtlEnableHw(_In_ RT_ADAPTER* adapter) {
    ...
    configPhyHardware(adapter);
    RtlSetupHw(adapter, adapter->intrMitigateValue, true);
...
void RtlDisableHw(_In_ RT_ADAPTER* adapter) {
    ...
    rtl8125_nic_reset(tp);
    hardwareD3Para(adapter);
    powerDownPLL(adapter);
...
void RtlSetupHw(_In_ RT_ADAPTER* adapter, UINT16 newIntrMitigate, BOOLEAN enableInterrupts) {
    ...
    //TODO: Tally Counter
    /*WriteReg32(CounterAddrHigh, (adapter->statPhyAddr >> 32));
    WriteReg32(CounterAddrLow, (adapter->statPhyAddr & 0x00000000ffffffff));*/
    ...
    if (tp->mcfg == CFG_METHOD_2 ||
        ...
        if (tp->mcfg == CFG_METHOD_4 || tp->mcfg == CFG_METHOD_5) {
            ...
            if (tp->DASH && !(csiFun0ReadByte(adapter, 0x79) & BIT_0))
    ...
    WriteReg16(RxMaxSize, adapter->mtu + (ETH_HLEN + ETH_FCS_LEN + VLAN_HLEN + 1));

    rtl8125_disable_rxdvgate(tp);

    /* Set receiver mode. */
    setMulticastMode(adapter, adapter->multicastMode);
    ...
    if (enableInterrupts) {
        ...
        WriteReg32(IMR0_8125, adapter->intrMask);
    }
    udelay(10);
...
void setPhyMedium(_In_ RT_ADAPTER* adapter)
{
    ...
    if (adapter->speed != SPEED_2500 && (adapter->speed != SPEED_1000) &&
        (adapter->speed != SPEED_100) && (adapter->speed != SPEED_10)) {
    ...
    tp->speed = adapter->speed;
    tp->duplex = adapter->duplex;
}
...
void enablePCIOffset99(_In_ RT_ADAPTER* adapter) {
    struct rtl8125_private* tp = &adapter->linuxData;
    u32 csi_tmp;
    ...
    case CFG_METHOD_4:
    case CFG_METHOD_5:
        csiFun0WriteByte(adapter, 0x99, tp->org_pci_offset_99);
        break;
    }
...
void disablePCIOffset99(_In_ RT_ADAPTER* adapter) {
    struct rtl8125_private* tp = &adapter->linuxData;
    ...
    case CFG_METHOD_4:
    case CFG_METHOD_5:
        csiFun0WriteByte(adapter, 0x99, 0x00);
        break;
    }
}
...
void initPCIOffset99(_In_ RT_ADAPTER* adapter) {
    ...
    enablePCIOffset99(adapter);
}

void setPCI99_180ExitDriverPara(_In_ RT_ADAPTER* adapter)
{
    struct rtl8125_private* tp = &adapter->linuxData;
...
void hardwareD3Para(_In_ RT_ADAPTER* adapter) {
    ...
    setPCI99_180ExitDriverPara(adapter);
...
void configPhyHardware8125a1(_In_ RT_ADAPTER* adapter)
{
    struct rtl8125_private* tp = &adapter->linuxData;
...
void configPhyHardware8125a2(_In_ RT_ADAPTER* adapter)
{
    struct rtl8125_private* tp = &adapter->linuxData;
    ...
    WriteReg16(EEE_TXIDLE_TIMER_8125, adapter->mtu + ETH_HLEN + 0x20);
...
void configPhyHardware8125b1(_In_ RT_ADAPTER* adapter)
{
    struct rtl8125_private* tp = &adapter->linuxData;
...
void configPhyHardware8125b2(_In_ RT_ADAPTER* adapter)
{
    struct rtl8125_private* tp = &adapter->linuxData;
...
void configPhyHardware(_In_ RT_ADAPTER* adapter)
{
    ...
    switch (tp->mcfg) {
    case CFG_METHOD_2:
        configPhyHardware8125a1(adapter);
        break;
    case CFG_METHOD_3:
        configPhyHardware8125a2(adapter);
        break;
    case CFG_METHOD_4:
        configPhyHardware8125b1(adapter);
        break;
    case CFG_METHOD_5:
        configPhyHardware8125b2(adapter);
        break;
    }
